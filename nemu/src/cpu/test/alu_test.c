...
void alu_test_add() {
  ...
  if (get_ref())
    printf(
        "\e[0;31mYou have used reference implementations, DO NOT submit this "
        "version!\e[0m\n");
}
...
void alu_test_imul() {
  ...
  for (i = 0; i < n; i++) {
    for (j = 0; j < n; j++) {
      ...
      assert((int32_t)res ==
             (int32_t)((res_asm_a & 0xffff) | ((res_asm_d & 0xffff) << 16)));
    }
  }
  ...
  for (i = 0; i < n; i++) {
    for (j = 0; j < n; j++) {
      ...
      temp_d = res_asm_d;
      assert((int32_t)res ==
             (int32_t)((temp_a & 0xffffffff) | ((temp_d & 0xffffffff) << 32)));
    }
  }
...
void alu_test_idiv() {
  ...
  for (i = 0; i < n; i++) {
    for (j = 0; j < n; j++) {
      ...
      asm("idivl %%ecx"
          : "=a"(quo_asm), "=d"(rem_asm)
          : "a"(aa.low), "d"(aa.high), "c"((uint32_t)b));
      assert(quo == quo_asm);
      assert(rem == rem_asm);
    }
  }
