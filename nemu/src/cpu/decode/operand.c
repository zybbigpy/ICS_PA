...
void operand_read(OPERAND *opr) {
  switch (opr->type) {
    case OPR_MEM:
      ...
      // thank Peiwen Yu from CS'16 for this fix
  ...
  switch (opr->type) {
    ...
    case OPR_MEM:
      if (opr->mem_addr.disp != MEM_ADDR_NA) {
        ...
        sprintf(disp_temp, "%s0x%x", displacement < 0 ? "-" : "",
                displacement < 0 ? -displacement : displacement);
      }
      if (opr->mem_addr.base != MEM_ADDR_NA &&
          opr->mem_addr.index == MEM_ADDR_NA) {
        sprintf(sib_temp, "(%s)", reg_names_l[opr->mem_addr.base]);
      } else if (opr->mem_addr.base == MEM_ADDR_NA &&
                 opr->mem_addr.index != MEM_ADDR_NA) {
        sprintf(sib_temp, "(,%s,%d)", reg_names_l[opr->mem_addr.index],
                opr->mem_addr.scale);
      } else if (opr->mem_addr.base != MEM_ADDR_NA &&
                 opr->mem_addr.index != MEM_ADDR_NA) {
        if (opr->mem_addr.scale != 0) {
          sprintf(sib_temp, "(%s,%s,%d)", reg_names_l[opr->mem_addr.base],
                  reg_names_l[opr->mem_addr.index], opr->mem_addr.scale);
        } else {
          sprintf(sib_temp, "(%s)", reg_names_l[opr->mem_addr.base]);
        }
      }
